...
from utils.chat_api import generate_messages, get_response_with_retry, parallel_get_embeddings
from utils.general import validate_and_fix_python_list
from prompts import prompt_memory_retrieval

MAX_RETRIES = 3

def generate_queries(question, query_num=3):
    input = [
        {
            "type": "text",
            "content": prompt_memory_retrieval.format(question=question, query_num=query_num),
        }
    ]
...
def retrieve_from_videograph(videograph, question, topk=3):
    ...
    query_embeddings = parallel_get_embeddings(model, queries)
